...
if __name__ == '__main__':
    ...
    args= ([(simulate_networks,(slice(1,2,1),slice(0,101,5), slice(1,2,1)),(test_net,seed_net))])
    p = Pool(1)
    res = (p.map(minimize,args))
    final_results = []
    for thrd in res:
    ...
    args= ([(simulate_networks,(slice(1,2,1),slice(1,2,1), slice(0,101,5)),(test_net,seed_net))])
    p = Pool(1)
    res = (p.map(minimize,args))
    final_results = []
    for thrd in res:
